...
void print_format(SDL_PixelFormat *fmt) 
{
        printf("format: %08x\n",fmt->format);
        printf("palette: %08xl\n", fmt->palette);
...
int main( int argc, char* args[] ) {
        ...
        window = SDL_CreateWindow( "Save surf", 0,0, SCREEN_WIDTH, SCREEN_HEIGHT,0);
        if( !window) {
        ...
        ssurf=IMG_Load("logo.png");
        if( !ssurf) {
                fprintf(stderr, "Unable to load image! SDL_Error: %s\n", SDL_GetError() );
                exit(3);
        }
        printf("loaded from logo.png\n");
        //print_format(ssurf->format);
        ...
        //printf("screen\n");
        //print_format(osurf->format);

        SDL_FreeSurface(ssurf);
        quad.w=osurf->w;
        quad.h=osurf->h;
        quad.y=0;
        quad.x=SCREEN_WIDTH-quad.w;
        SDL_FillRect( screenSurface, NULL, SDL_MapRGB( screenSurface->format, 17, 40, 57 ) );
        SDL_BlitSurface(osurf, NULL, screenSurface,&quad);
        
        ssurf=IMG_Load("setting-tiko.png");
        if( !ssurf) {
                fprintf(stderr, "Unable to load image! SDL_Error: %s\n", SDL_GetError() );
                exit(3);
        }
        
        printf("loaded from setting.png\n");
        osurf = SDL_ConvertSurface(ssurf,screenSurface->format, 0);
        if( !osurf){
                fprintf(stderr, "Unable optimize surface SDL_Error: %s\n", SDL_GetError() );
                exit(4);
        ...
        ssurf=SDL_PNGFormatAlpha(screenSurface);
        SDL_SavePNG(ssurf,"bg2.png");
        SDL_Delay( 2000 );
        SDL_FreeSurface(ssurf);
